...
st.caption(
    "국민은행 신한은행 하나은행 상품 설명서 기반 |"
    f'fine-tuning 모델 "{FINETUNED_MODEL}"'
)
...
if 'display_messages' not in st.session_state:
    st.session_state.display_messages = []          # 화면 표시용 dictionary 리스트


# ------------------ 사이드바: 사용자 프로필 입력 -----------------
with st.sidebar:
    st.header("ℹ️ 내 금융 프로필")
    st.caption("입력 시 더 정확한 진단이 가능합니다.")

    target_bank = st.selectbox(
        "희망 은행",
        ["선택 안함","국민은행","신한은행","하나은행", "비교해서 추천"]
    )
    ...
    credit_score = st.number_input(
        "신용점수", min_value=0, max_value=100, value=0, step=10
    )

    existing_loan = st.number_input(
        "희망 전세금(만원)", min_value=0, max_value=50_000, value=0, step=10
    )

    target_amount = st.number_input(
        "희망 대출액(만원)", min_value=0, max_value=200_000, value=20_000, step=500
    )

    if st.button("📊프로필 기반 진단", use_container_width=True):
        if annual_income > 0:
            # 프로필 문자열 생성 (사용자 입력값 재정의)
            profile_lines = [
                f"희망 은행: {target_bank if target_bank !='선택 안함' else '미정'}",
            ...
            st.session_state.display_messages.append({'role':"user","content":displaying_msg})
            st.rerun() # HumanMessage의 업데이트 내용을 반영함 
...
# - chat_message(): streamlit에 대화 메시지 출력 
...
for msg in st.session_state.display_messages:
    avater = "👨‍🦱" if msg['role'] == 'user' else "🏠"
    with st.chat_message(msg['role'], avater=avater):
...
if st.session_state.messages and isinstance(
    st.session_state.message[-1], HumanMessage
):
