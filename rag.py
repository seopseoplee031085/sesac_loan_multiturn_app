...
def get_vectorstore(
    ...
    if vectorstore_exists:
        try:
            ...
            if vectorstore._collection.count() >0:
                print(f'기존 vectorstore 로드 완료'
                      f'(문서 수: {vectorstore._collection.count()})')
                return vectorstore
            else: print('!!! vectorstore는 존재하지만 문서가 없습니다. 재생성합니다.')
        except Exception as e:
            print(f'!!! vectorstre 로드 오류: {e}. 재생성합니다.')
    
    else:
        print("!!! vectorstore가 존재하지 않습니다. 새로 생성합니다.")
    
    # 새 vectorstore 생성 코드 작성
    # 1) 3개 은행 PDF 로드
    all_docs = []
    for pdf_path in PDF_PATH:
        # 파일이 해당 경로에 존재하지 않을 경우
        if not os.path.exsist(pdf_path):
    ...
    if not all_docs:
        # raise: 예외를 발생시켜서 프로그램을 중단시키는 키워드
...
def search_context(
    ...
    retriver = vectorstore.as_retriver(search_kwargs={"k":k})
    contexts_with_metadata = retriver.invoke(input_data)
    contexts_only_text     = []
...
# 검색된 context를 하나의 문자열로 합치는 헬퍼 정의
def build_context_string(contexts_only_text: list[str])-> str:
    """검색된 청크들을 개행으로 연결하여 프롬프트 삽입용 문자열로 반환시킴"""
    return "\n".join(contexts_only_text)

# 단독 실행 테스트
#  - 해당 rag.py 파일을 직접 실행시켰을 경우 파일명은 "__main__"형태를 띔
if __name__ == "__main__":
    import sys

    # sys.argv(): 
    #  - 터미널에서 Python 파일을 실행할 때 같이 넘겨주는 값들을 담고 있는 리스트 
    #  - 파이썬에서 명령줄 인자(command-line arguments)를 받는 방법
    #  - 코드 간략 설명: 만약 input_data에 
    input_data = (
        sys.argv[1]
        if len(sys.argv) > 1
